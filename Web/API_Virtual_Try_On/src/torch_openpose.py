...
model_body25 = r"D:\GITHUB\Vouge\Posing\body_25.pth"

class torch_openpose(object):
    def __init__(self, model_type):
        if model_type == 'body_25':
            self.model = bodypose_25_model().to(device)
            self.njoint = 26
            self.npaf = 52
            ...
            self.njoint = 19
            self.npaf = 38
            self.model.load_state_dict(torch.load(model_coco, weights_only=True))
